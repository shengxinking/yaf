    ...
    char                          *str_prefix)
{
    ...
    fprintf(fp, "\t+++ subTemplateMultiListEntry %d +++\n", index);

    fprintf(fp, "\theader:");
    fprintf(fp, "\tcount: %3d", entry->numElements);
    fprintf(fp, "\t\ttid: %5d (0x%02x)\n", entry->tmplID, entry->tmplID);
    ...
    char                     *str_prefix)
{
    ...
    fprintf(fp, "%s\theader:", str_prefix);
    fprintf(fp, "\tcount: %3d", stl->numElements);
    fprintf(fp, "\ttid: %5d (0x%02x)", stl->tmplID, stl->tmplID);
    fprintf(fp, "\tsemantic: %3d\n", stl->semantic);
    ...
    char            *str_prefix)
{
    ...
    fprintf(fp, "elements: %3d", stml->numElements);
    fprintf(fp, "\tsemantic: %3d\n", stml->semantic);
    ...
    char              *str_prefix)
{
    ...
    if (!ie) {
        fprintf(fp, "count: %3d", bl->numElements);
        if (bl->semantic) {
            fprintf(fp, "\tsemantic: %5d\n", bl->semantic);
        } else {
            fprintf(fp, "\n");
        }
        return;
    ...
    if (bl->numElements == 0) {
        fprintf(fp, "%s empty list\n", str_prefix2);
    }

    while ((data = FBBLNEXT(bl, data))) {
        fprintf(fp, "%s %d : ", str_prefix2, i);
        i++;
    ...
    char         *prefix)
{
    ...
    fprintf(fp, "%s--- data record %d ---\n", prefix, rec_count);
    fprintf(fp, "%sheader:\n", prefix);
    fprintf(fp, "%s\ttid: %5u (0x%02x)", prefix, tc->tid, tc->tid);
    fprintf(fp, "\tfield count: %5u\n", tc->count);
    fprintf(fp, "%sfields:\n", prefix);

    for (i = 0; i < tc->count; i++) {
        ie = fbTemplateGetIndexedIE(tmpl, i);
        fprintf(fp, "%s\t(%d) %30s \t : \t", prefix, ie->num,
                ie->ref.canon->ref.name);
